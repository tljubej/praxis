...
/// this frame's slots (ADR-100). Computed from the `#[repr(C)]` layout so it
...
/// locals, which is most of them, and the point of ADR-100 is that the common
...
pub(crate) fn lower_function<M: Module>(
    ...
    // Prologue (cont.): claim this frame's run of shadow slots (ADR-100). The
...
struct SpillCtx<'a> {
    ...
    /// stack (ADR-100) — not a frame object. The spill indexes it directly, and
...
/// Emit the shadow-stack epilogue (ADR-019, ADR-100): give this frame's slots
