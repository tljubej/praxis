...
/// v15 (finding §3.3, ADR-100): the shadow stack is one contiguous region owned
...
/// shadow stack (§12.4, ADR-019, ADR-100). Called by every allocating
...
mod tests {
    ...
    #[test]
    fn version_is_fifteen_after_the_contiguous_shadow_stack() {
    ...
    // deliberately does not null-check the context (ADR-100 — the check cost
...
"#;
    ...
    fn a_panic_dummy_is_only_returned_where_a_fault_check_can_follow() {
        ...
        // it. ADR-100 inlined it and its pop, and with them gone every
