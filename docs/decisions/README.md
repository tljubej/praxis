...
- [ADR-121: A slot that provably holds a scalar is not a box, and changing what a slot holds is not substitution (W8-S1; the wave-5 gate, opened)](./121-a-slot-that-holds-a-scalar-is-not-a-box.md)
...
- [ADR-137: A deferred receiver resolves in rounds, and the channel runs to a fixpoint](./137-a-deferred-receiver-resolves-in-rounds-and-the-channel-runs-to-a-fixpoint.md)
- [ADR-138: A container orders by the value, and not by its printing](./138-a-container-orders-by-the-value-and-not-by-its-printing.md)
- [ADR-139: A pattern's name is a name in the frame, and a slot no pattern names is a temp](./139-a-pattern-name-is-a-name-in-the-frame.md)
- [ADR-140: A counted `repeated` is bounded, so something can follow it](./140-a-counted-repeated-is-bounded-so-something-can-follow-it.md)
- [ADR-141: A character is one token, and a literal is a load](./141-a-character-is-one-token-and-a-literal-is-a-load.md)
- [ADR-142: *reserved* — handover 31's sized-collection package; the package landed as ADR-146 instead and this number was never written. Spent, not recycled.](./142-slug.md)
- [ADR-143: The `to_text` family is `Int`, `Float` and `Char`](./143-the-to-text-family-is-int-float-and-char.md)
- [ADR-144: A sequence of `Text` joins, and a sequence of `Char` becomes one](./144-a-sequence-of-text-joins-and-a-sequence-of-char-becomes-one.md)
...
- [ADR-156: The site is the page with the book under it, deployed by the recipe you preview with](./156-the-site-is-the-page-with-the-book-under-it.md)
