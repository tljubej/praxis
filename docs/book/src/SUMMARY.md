# Summary

[The Praxis Book](README.md)

---

# Getting started

- [Installing Praxis](getting-started/install.md)
- [Your first program](getting-started/first-program.md)
- [The command line](getting-started/cli.md)

# The language

- [A file is a program](language/program-structure.md)
- [Bindings and shadowing](language/bindings.md)
- [Scalars](language/scalars.md)
- [Numbers](language/numbers.md)
- [Text and Char](language/text.md)
- [Control flow](language/control-flow.md)
- [Functions and closures](language/functions.md)
- [Records](language/records.md)
- [Enums and Option](language/enums.md)
- [Pattern matching](language/pattern-matching.md)
- [Collections](language/collections.md)
- [Pipelines](language/pipelines.md)
- [Grids and graphs](language/grid-and-graphs.md)
- [The prelude](language/prelude.md)
- [The method catalog](language/method-catalog.md)

# Reading input

- [The `read` expression](input/read.md)
- [Atomic parsers](input/atoms.md)
- [Templates and captures](input/templates.md)
- [Structural parsers](input/structural.md)
- [Whitespace, lines and positions](input/whitespace.md)
- [How a parser gets its type](input/type-derivation.md)
- [When a parse fails](input/faults.md)
- [Cookbook: input shapes](input/cookbook.md)

# Type inference

- [What inference does](types/model.md)
- [Generalization](types/generalization.md)
- [Method resolution](types/method-resolution.md)
- [Capabilities](types/capabilities.md)
- [Records without names](types/structural-records.md)
- [Reading a type error](types/errors.md)
- [Inference in the editor](types/editor.md)

# The crash debugger

- [The fault model](debugger/faults.md)
- [Entering the debugger](debugger/entering.md)
- [Breakpoints](debugger/breakpoints.md)
- [The full-screen debugger](debugger/tui.md)
- [Command reference](debugger/commands.md)
- [Evaluating expressions](debugger/expressions.md)
- [Inspecting the input parser](debugger/parser.md)
- [A walkthrough](debugger/walkthrough.md)
- [Noninteractive mode](debugger/noninteractive.md)

# Tooling

- [Editor support](tooling/editors.md)
- [Diagnostic codes](tooling/diagnostics.md)

# Under the hood

- [The compiler pipeline](internals/pipeline.md)
- [The object heap and the collector](internals/heap.md)

---

[Appendix A: Complete programs](appendix/programs.md)
[Appendix B: Grammar](appendix/grammar.md)
