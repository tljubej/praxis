...
**REP-15 is done** (ADR-066). It was the last P0, it never got a stage — it
arrived after every stage that could own it was written — and it took the vertical
slice the row asked for: a decision (the iteration protocol), four runtime
wrappers, a total `IterPlan` in MIR, and gates at every level. **Two rows are
`unscheduled`: REP-19 and REP-21**, both below P0.
