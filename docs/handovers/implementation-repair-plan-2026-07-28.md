...
schedules is closed, so the suffix is on every row **but one** — the next
paragraph names it — and it went on in **one
...
**With one stated exception, which is where the rule would otherwise read
false.** **REP-52** and **REP-53** name **S9** in their Stage column while S9 is
closed. They are not rows S9 left undone: they are scheduled *behind* **MIR-10's
owed half** — the verifier rule "a faulting instruction is followed by a
`CheckFault`", which is why MIR-10 is the one §4 row reading `PARTIAL — part
owed` — and S9 is the stage that owns MIR-10, so it is the stage the two rows
name. Taking either without that rule fixes one site and leaves the class, which
is what both rows say. Read them as MIR-10's remainder, not as S9's.

**The open rows are REP-33, REP-52, REP-53, REP-54, REP-55, REP-56, REP-57,
REP-58, REP-60's stdin half and REP-64, plus the `_sub`/`_mul` half of REP-46.**
No number is given here on
...
and REP-54/REP-55 by its fifth; **REP-60** was registered by the final audit and
its `--input` half is fixed; **REP-64** is the one open row that is a *silent
wrong answer* (`f += 2.0` on a `Float`), found while landing ADR-085 in the two
lines that carry it. **Everything else in this register is done.**
...
row in this register up to REP-25 is done**: REP-10 closed S25's schedule (ADR-069),
...
fix. **The register runs to REP-64 now.** REP-61 … REP-63 came from a user
reporting one confusing diagnostic — `"asd" + "qwe"` saying `expected Text, found
Int` — which turned out to be three independent defects wearing one message: the
swapped halves, a caret one column wide of its expression, and a 0-based column
in the header. REP-64 came from ADR-085's own implementation. REP-26 … REP-34 came from reading the
design doc's own programs against the front end; REP-36 … REP-48 from three
fresh-eyes sweeps of the runtime, the front end and end-to-end semantics, plus
one flaky test; REP-49 from REP-37's own fix, which found it; REP-50 … REP-53
from the review of that block; and REP-54 … REP-59 from S20's repair pass and its
...
and `12`). **S25 is closed**, and with it every row this register held at the
time. The register has reopened since — for REP-26 … REP-53 across this
session's sweeps and reviews, and for S20's REP-54 … REP-59 — see the note at the
top of this section.
...
**REP-35 is new and belongs to no stage.** It was found while re-reviewing S19's
nested-template span rebasing: the spans were right and the *carets* were not,
because the renderer read a byte offset as a display column. It is pre-existing
— it reproduces byte-for-byte against `main` at `d8179e1` — and it is registered
here rather than under S19 because it is nobody's stage and it invalidates every
span-precision claim about a line holding non-ASCII text. It landed with the
S19 round-three fixes.

**REP-36 … REP-48 are the register's third block**, found by three fresh-eyes
sweeps — a runtime+GC hunt, a front-end hunt, and an end-to-end hunt that ran
real Praxis programs — plus one flake. **REP-49 is not from the sweeps at all**:
REP-37's own width assertion found it on the first `cargo test` after it landed,
which is the argument for making a rule structural rather than fixing its
instances.

They divide by *how the defect hid*, and the division is worth keeping. Four were
**wrong answers a test could have caught and did not**, because every test in
range asserted the wrong quantity: the grid rotations were covered by three tests
that assert only `width() * 10 + height()`, and both rotations of a 3×2 grid have
identical dimensions (REP-36); `filter_map` was pinned by a test asserting `map`'s
answer, and its premise was the comment admitting the gap (REP-38); `find` was
pinned by a test whose *name* was the finding, `pipeline_position_is_alias_of_find`
(REP-39); and the `Float` rendering was covered by a descriptor test whose one
value is `2.5`, which carries a `.` and passes under either rule (REP-44). Three
were **claims in the code that were false about the language**: `INT` written
where "unknown" was meant (REP-41, REP-42), and a comment about Rust's
round-tripping applied to a language where `1` is not a `Float` (REP-44 again).
Two were **the ABI's own rules broken in one place each** — an eight-byte read of
a one-byte payload (REP-37, and REP-49 behind it), and an unchecked add inside
`extern "C"` (REP-43). Two were **the manifest disagreeing with the code it
describes** (REP-45) or **the design doc describing surface that was never built**
(REP-46). One was a **silent reinterpretation** rather than a report (REP-47). One
was **dead code that would have been wrong if reached** and is recorded at that
size rather than inflated (REP-40). And one was a **test whose observation was a
probability** (REP-48).

Every one of the thirteen was re-reproduced against `9727b34` or later before
being fixed — which mattered: `main` had already gained S18, S21 and REP-26 …
REP-34 by the time these were written down, and REP-38's claim in particular had
to be re-measured *after* S21 rewrote the whole block it lived in. S21 moved the
stage from an eager lowerer into `Step::FilterMap` and moved the defect with it.
...
| REP-20 | P1 | S | **S25 — done in two rounds** (`b495e93`, `7938186`) | input-parser | S25 REP-18 | **A template literal that begins with a space can never match.** `` `{a:int} -> {b:int}` `` faults `ParseFailed` at the `-` on every input. `walk_template` honours a literal's whitespace policy *before* matching its bytes, and `scan_template` left the leading space run in the text as well, so the same space had to be consumed twice. **§3.3's own template is exactly that shape.** Fixed in the scanner at `b495e93`: the leading run becomes the literal's `WsPolicy` and leaves the text, a run *inside* a literal is untouched, and `\x20` is still the escape for a space that must match exactly. **The trailing half of this row was claimed and not delivered, and the final audit caught it.** This row read "a space/tab run at either *end* of a literal is the policy and leaves the text"; what `b495e93` did was `trim_matches` **both** ends and derive a policy from the **leading** one only — a literal has one policy slot and it sits in front of the text — so the trailing run was stripped out of the text and represented **nowhere**. It was neither required nor consumed. Measured at `5d15a44`: `` `x: {a:rest}` `` accepted `x:hello`, where the leading spelling correctly faults; over `x: hello world` the capture came back as `" hello world"`, twelve bytes, one of them the template's own; `` `{a:int}-> {b:int}` `` matched `1->2`, the exact input ADR-079 Decision 4 says now faults. **A silent wrong answer in ordinary programs** — the AoC shape `` `Card {id:int}: {body:rest}` `` compared `body` against the file's own bytes and answered `false` — and it broke **D10's flagship construct** outright: `` `op: {g:choice(Plus: `plus {n:int}`, …)}` `` over `op: plus 3` faulted `expected literal "plus"` at the space, and so did a nested template body and `` `x: {a:char}` ``. **Why nothing caught it:** every template in the corpus and in `tests/input-parsers` puts an `int` or a `word` behind its trailing run, and both self-trim in `walk_atomic`; `char`, `text` and `rest` deliberately do not, and S20 had already deleted the premise the strip was justified by ("every capture consumes leading whitespace before it walks"). So the scanner was still relying on a rule the runtime no longer had. **Landed at `7938186`**: `flush` emits the trailing run as **its own part** — an empty literal carrying `SpaceRun`, which is the representation `` `{a:int} {b:int}` `` already lowered to and every consumer already handles — and a literal that strips to nothing stays **one** part, or `` `{a:int} {b:int}` `` would demand two runs. ADR-079 Decision 4 is amended, including its "That input now faults", which held for the leading spelling only. **Gates:** `a_template_literals_trailing_space_run_is_its_policy_too` (`jit.rs`) asserts the exact bytes of a `rest`/`text` capture, the `1->2` refusal against the trailing spelling and the flexible run that does match, the `choice`/nested-template/`char` shapes, and the two things that must **not** change — a trailing run in the *input* that no part asked for is still nobody's (ADR-078, `walk_exact`), and a whitespace-only literal still asks for one run; `a_literals_edge_whitespace_is_its_policy_and_not_its_text` (`scan.rs`) asserts the part sequence at the scanner, where the rule lives; `tests/input-parsers/template_trailing_run.px` is the corpus fixture the corpus had no shape of. Each was verified red without the fix. |
...
| REP-35 | P2 | S | **done** (S19 round 3) | fe-cli-dbg | S19 re-review | **The diagnostic renderer treats a span's byte offset as a display column.** `praxis_source::snippet` padded the caret with `seg_start - line_start` bytes, sized the caret run in bytes, and printed the byte column in the `path:line:col` header — so every multi-byte character *earlier* on the line pushed the caret one column right per extra UTF-8 byte, and a multi-byte character *under* the caret drew one caret per byte. `let y = λλ + nosuchname` put the `N001` caret two columns late and two columns wide over two characters; the same on every `I0xx`, `Y0xx` and `P0xx` on any line holding non-ASCII text. Pre-existing and not S19's — reproduced byte-for-byte against `main` at `d8179e1` — but it makes **every span-precision claim about a non-ASCII line unverifiable as rendered**, which is how it was found (while checking S19's nested-template span rebasing). **Fixed** in `praxis_source::snippet` only: one `char_width(text, from, to)` counts characters between two byte offsets and is used for the header column, the caret padding and the caret run. `LineMap`/`LineCol` are **untouched** — `col` there is a byte column that `linecol_to_offset` round-trips, and it is the LSP's unit; a display column is the renderer's concern, so the conversion lives at the point of printing. Gated by a snapshot on `λλ`, both as text before the caret and as text under it. |
| REP-36 | P1 | S | **done** | rt | runtime sweep + end-to-end sweep, independently | **`Grid.rotate_left()` and `Grid.rotate_right()` each perform the other's rotation.** With §6.4's convention (x rightward, y downward), a 3×2 grid `abc / def` turned counter-clockwise is `cf / be / ad`, so `rotate_left().get(0, 0)` must be `c`; it answered `d`, which is the *clockwise* result, and `rotate_right()` answered `c`. Reproduced at `9727b34` with `praxis run`. The two bodies held each other's index formula and each doc comment stated the formula belonging to the other function, so reading either one in isolation looked right. **Why nothing caught it:** the only three tests (`grid_rotate_left_changes_dimensions`, `grid_rotate_right_changes_dimensions`, `grid_rotate_four_times_is_identity`) assert `width() * 10 + height()`, and the two rotations of a 3×2 grid have *identical* dimensions — a rotation test that never reads a cell cannot see direction at all, and four turns is the identity whichever way it turns. **Landed** by exchanging the two index computations and rewriting both comments to derive the formula from the convention rather than restate it. The gate is `grid_rotate_left_and_right_turn_in_opposite_directions` in `jit.rs`, and both halves are in one answer (`1` for left, `10` for right, `11` for both) precisely because a fix that swapped the two bodies *again* passes either half alone; the grid is asymmetric in both axes so no transpose, flip or 180° turn produces the same reading. It answers `0` before the fix. |
| REP-37 | P1 | S | **done** | rt | runtime+GC hunt | **The graph oracle read a closure's `Bool` answer as an `i64`, seven bytes past the object.** `ClosureOracle::is_goal` checked the descriptor and then called `int_payload` — an eight-byte read — but a `Bool`'s payload is **one** byte (`BoolPayload = u8`, and `BOOL` is `TypeDescriptor::builtin::<BoolPayload>`), so `BlockLayout::of` lays the block out as `payload_offset(24) + 1` and the read consumed seven bytes of alignment padding the bump allocator never initialized. The safety comment said "the payload is a `Bool`'s `i64`", which is the false half. Every `bfs_distance` / `a_star` / `flood_fill` goal predicate went through it, and the answer depended on what malloc had left there — a `false` whose padding was non-zero makes `bfs_distance(0, …, |s| s == 5)` answer `Some(0)`. **Landed as ADR-060's rule made structural rather than as a corrected literal**, which is the shape S23's `Payload<T>` already set: `Payload::read` takes the width from `T`, and `Payload::new` proved at *const-evaluation* time that `T`'s layout is the descriptor's, so no call site names a width at all; `read_scalar(r, handle)` pairs that with the descriptor-identity check the old code did by hand and answers `Option<T>`, so reading the wrong type and reading the right type at the wrong width are both unspellable. `int_payload` keeps its eight-byte read for the arithmetic wrappers and now `debug_assert`s the operand's width, so the next site that reaches for it on a narrower payload fails loudly. The gate is `a_graph_goal_predicate_reads_a_bool_at_a_bool_s_width`. **Its first spelling was not a gate**, and a reviewer measured it: an end-to-end walk over the immortal `false` passes with both halves of the fix removed, 0 failures in 300 runs, because the arena never produces the padding the defect needs — a `Bool` block is `payload_offset + 1` bytes, bumpalo rounds the next base down to eight, and the seven bytes between are slack nothing writes, so fresh pages read back zero and the eight-byte read answered *correctly* every time. The gate now owns its operand: `DirtyPaddedBool` is a `Bool` laid out the way `alloc_raw` lays one out, in the test's own memory, payload byte `0x00` and the next seven bytes `0xFF`, headed by a freshly minted `HeapId` so the collector's provenance check skips it. An eight-byte read answers `0xFFFF_FFFF_FFFF_FF00` and a read at any offset past byte zero answers `0xFF` — both are "goal reached at the start state", both give `Some(0)` — so the test fails if the width **or** the offset is wrong, and both mutations were run. The old walk is kept beside it as `a_graph_goal_predicate_that_is_false_everywhere_finds_nothing`, labelled a companion. |
| REP-49 | P1 | S | **done** | mir | REP-37's own guard, during this session | **A `Bool` pattern compared eight bytes of a one-byte payload.** `emit_pattern_test` put `Lit::Bool` in `Lit::Int`'s arm, so `match b { true => …, false => … }` emitted `ExtractScalar { scalar: Int }` on both operands — `praxis_int_load`, an eight-byte read — against a payload that is **one** byte. The seven bytes above it are the block's alignment padding, which the bump allocator never writes, so `true` and `false` were told apart by uninitialized memory: `match false` takes the `true` arm on any run where the two immortals' padding happens to agree, which for freshly-`alloc`'d chunks is not a remote possibility. **Not in the twelve; REP-37's `int_payload` width assertion found it** on the very first `cargo test` after it landed, which is the argument for making a rule structural rather than fixing its instances. **Landed** by reading each operand at the payload's own width — `ScalarKind::Bool`, so `praxis_bool_load`; the `Int` arm is untouched, since the fix is a width and not a rewrite of literal matching. The gate is `a_bool_pattern_reads_its_scrutinee_at_a_bools_width`, and it asserts the **instruction**, because the behaviour cannot say it: `match true` is right whenever the paddings differ, and comparing `true` against itself reads one address twice and is right for the wrong reason. |
| REP-38 | P1 | M | **done** | mir + stdlib | end-to-end semantics hunt | **`filter_map` did not filter — it was lowered as `map`, so the `None`s survived into the result.** `v.filter_map(\|x\| if x > 2 { Some(x * 2) } else { None }).collect()` answered `[Some(6), None, Some(8), None, Some(10)]` where §6.3 asks for `[6, 8, 10]`. **Re-reproduced at `9727b34`, after S21**: ADR-071's recursive `Chain` rewrote the whole block and did not touch this — S21 moved the stage from an eager lowerer into `Step::FilterMap` and moved the defect with it. The row was `Stability::Stable` with a doc admitting the gap ("modeled as map-keep"), and nothing in the compiler warned. The second-order symptom is worse than the first: appending `.sum()` reports `error[Y001]: expected Int, found Option[Int]`, so the user is told their *sum* is wrong and never that `filter_map` did not filter. **The cause was the type, not the loop.** The row typed the closure `(T) -> U` for an unconstrained `U`, which is `map`'s shape, so at runtime nothing said "this element mapped to nothing" — the old comment says exactly that, and it was true when it was written. **S18's `Option` (ADR-076) retired it**, so this is a repair to a stated rule rather than a new decision and needs no ADR: the row is `(T) -> Option[U] -> Vec[U]`, and `Step::FilterMap` reads the answer's tag with the same `EnumTag`/`EnumPayloadGet` pair a `match` on `Option` emits, branching `None` to the innermost continue edge exactly as `Step::Filter` does on a false predicate. A closure answering a bare `U` is now `Y001` **at the closure**, which is where the mistake is. **`pipeline_filter_map_keeps_results` pinned the defect and is rewritten, not deleted** (§8.2): it asserted `[1,2,3].filter_map(\|x\| x * 2).sum() == 12`, which is `map`'s answer and passed because `filter_map` *was* `map`; its premise was that comment. Its replacement `filter_map_drops_the_nones_rather_than_carrying_them` asserts a sum a surviving `None` cannot even type-check, and two further gates cover the all-`None` result and a `filter_map` between two other stages. §6.3 now writes the closure's shape. |
| REP-39 | P1 | M | **done** (ADR-082) | mir + stdlib | end-to-end semantics hunt | **`Vec.find(pred)` answered the element's *index*, making it an exact duplicate of `position`.** Both rows were `result: Int` with the doc "Index of the first matching element, or -1 on miss" — `position`'s adding "(alias of find)" — and MIR handled `Sink::Find(_) \| Sink::Position(_)` in one arm with one `-1`-seeded accumulator. §6.3 lists them as **two** operations, so the list named one operation twice. `v.find(\|s\| s == "beta")` on `["alpha","beta","gamma"]` answered `1`; a program could not reach the element **at all**, because the row's result is `Int` whatever the receiver holds. ADR-029 decision 5 decided the `-1` sentinel and nothing else — that `find` answers an index is an implementation shortcut that reached the catalog and then the docs. And the sentinel is *in band*: `v.find(\|x\| x < 0)` on `[10, -1, 30]` and `v.find(\|x\| x > 100)` both answered `-1`. **Decided in ADR-082**, which S18 made possible: `find` answers `Option[T]` and `position` answers `Option[Int]`, both stopping at the first match, and the whole difference between them is what they record when they stop. The sink carries a seen-flag — the one `min`/`reduce` already use — but *not* the `EmptyCollection` fault: a `find` that matches nothing is ordinary absence (§4.7), and an empty `min` is a caller mistake (D1). The `-1` spelling is retired rather than kept beside it, because a caller who reaches for a bare `Int` gets a plausible index back and cannot tell a hit from a miss any better than before. **Four `jit.rs` tests pinned the old contract and are rewritten, not deleted** (§8.2) — including `pipeline_position_is_alias_of_find`, whose *name* was the finding. The gate is `find_answers_the_matching_element_not_its_index`, on a vector where the element and its index are different numbers, plus `find_reaches_a_non_int_element`, the assertion no arithmetic could make before. Three per-stage-index tests (ADR-071) grew a `match` and measure exactly what they did. |
| REP-40 | P2 | S | **done** (dead code, deleted) | mir | runtime sweep | **`lower_seq_fold` returned the seed and never invoked the closure** — a doc comment promising "loop, threading an accumulator through `f(acc, item)`" over a body that discards the source, the index and the closure. It sat inside the eager per-combinator fallback ADR-029 decision 1 kept "as a fallback for any chain the recognizer declines, so a regression here can never break the eager path", beside a `_` arm answering the Unit singleton. **Measured before deciding**, because the alternative was implementing the fold: the decline path was replaced with a panic naming the method and the whole workspace was run — **1517 tests pass**, so nothing in the suite, the corpus or the design-doc fences reaches the fallback, which matches S21's own note that all 23 registered `Intrinsic` names are classified. So the finding is **dead code that would be wrong if reached**, and it is recorded at that size rather than inflated into a second lowering of an operation the fused path already has. **Landed as a deletion**: `lower_pipeline_combinator`, the `lower_seq_*` family and `emit_index_loop` are gone (`alloc_empty_vec` stays — the fused emitter calls it), and a declined chain is now an ICE naming the method, which is a bug report instead of a seed. **The gate is not red on `main`, and that is the row's own conclusion rather than a shortfall** — deleting unreachable code changes no behaviour. What replaces the net is the property that made it unnecessary, asserted: `intrinsics_are_all_recognized_so_there_is_no_second_lowering` walks the catalog and, for every row lowered as an `Intrinsic` — which by construction has no runtime symbol, so the recognizer is its only route to code — asserts the recognizer classifies it. Deleting `classify_sink`'s `reduce` arm turns it red. |
| REP-41 | P1 | M | **done** | rt | runtime+GC hunt | **Five `praxis_*_new` wrappers turned the codegen's "element type unknown" null descriptor into `&INT`, mislabelling the collection.** The contract above `collection_element_descriptor_for` is explicit — a null descriptor is the honest encoding of "this lowering has no static element type", and every wrapper reads it that way — and `praxis_vec_new` says in its own comment why ("spelling it `INT` is what made an empty `Vec[Float]` claim to hold `Int`s", P0-11). `Set`, `Counter`, `Map`'s key and the two heaps each replaced the null, because the payload field was a non-nullable `&'static TypeDescriptor` and there was nothing else to put there; and unlike `Vec`/`Deque`/`Grid`, none of the five ever self-corrected. The label was then **dispatched through**: `praxis_counter_keys` tagged its snapshot `Vec` with it, so a `Text`-keyed `Counter` answered `[0]` — the first eight bytes of a `TextPayload` formatted as an integer; `min_heap_format` rendered a `Float` through `INT`'s formatter; `set_hash`/`map_hash`/`counter_hash` hashed payloads through a type they do not have, and `map_equals` compared two payloads with a third type's `equals`. **ADR-066 decision 5 was already the rule and is applied rather than a new default chosen**: a null slot is legal and means "the value's own descriptor answers". The six payload fields are `*const TypeDescriptor` with an `Option`-answering accessor, as `VecPayload` has had since P0-11 — and, the half that matters more, **every element-wise dispatch now reads the value's own descriptor**: a `DynamicKey` and a `HeapEntry` have always carried one (`set_format` and `HeapEntry::cmp` were already using them), so the label survives only as what the construction site knew. `vec_of` adopts the first member's descriptor when the source's label is null, because a `Vec`'s null means *empty*. The gate `a_collection_with_no_static_element_type_does_not_claim_int` asserts **both** halves — the five labels absent, and a `Text` key, a `Float` heap element and a `Text` set member rendering as themselves — because a test that checked only the label would pass a fix that left the dispatch wrong. |
| REP-42 | P2 | S | **done** | rt | runtime sweep | **`praxis_map_new` hardcoded `INT` as every `Map`'s value descriptor**, under a comment arguing it was "sound for now: values are uniform GcRefs and format via the descriptor adopted from the first inserted value". The soundness held only while every reader ignored the field, and the progress doc records where it did not: REP-18's `keys()`/`values()` pair is built in MIR rather than in the runtime *because* "a `Map`'s payload records `INT` as its value descriptor unconditionally, so a `Map[Text, Text]`'s pair would read its value as an `i64`" — a live constraint on where future work may read the payload, not a note. The deeper problem is that `INT` was **also the spelling of "unknown"**, so the adoption beside it could not be written correctly: "adopt if the label says `INT` and the value is not an `Int`" cannot tell a `Map` that really holds `Int`s from one that has never been told anything. REP-41 made null representable, so the two states are two values: the slot starts null, the first insert adopts the value's own descriptor exactly as `praxis_vec_push` does, and a later value of another type un-learns it rather than faulting — unreachable for a well-typed program, and `MapInsert` is a non-faulting row whose caller emits no check, so forgetting is the only safe direction available to it. The gate `a_map_does_not_claim_its_values_are_ints` asserts the empty map's slot is absent (red on the old spelling) and — because a null alone would pass a fix that never learns anything — that the first insert teaches it `Text`, that `values()` then renders `[vv]`, and that a `Map` genuinely holding `Int`s reports `Int`: the assertion the old spelling could not make at all.  **Collateral, found by a reviewer and fixed in the same block**: making the label honest made an *empty* `Map`'s `values()` unequal to an equally-typed empty `Vec`, because `same_element` was pointer identity and null is not `INT`. Measured against the merge base — `true` at `9727b34`, `false` at `11dd316`. The old `true` was an accident of the hardcoded `INT` and **not a rule**: the same program over a `Map[Text, Text]` answered `false` both before and after. The fix is in the comparison, not in a reinstated guess, which is the mistake this row and REP-41 both exist to remove: `same_element` now treats a **null slot as agreeing with anything**, which is ADR-066 decision 5 — a null slot is the absence of a label, and what answers instead is the value's own descriptor, of which an unlabelled collection has none. RT-10 is untouched: two collections that have each been told their element type must still agree, and the length check every caller performs is what decides. Gate: `an_unlearned_element_label_does_not_make_two_empty_collections_unequal`, red on pointer identity. |
| REP-43 | P1 | S | **done** | rt + stdlib | runtime+GC hunt | **`Counter.inc` added without a checked operation: it panicked across `extern "C"` in debug and wrapped silently in release.** Every other integer computation in the ABI file is checked and faults — `checked_int_binop!`, `checked_neg`, `checked_abs`, `checked_gcd`'s `i128` widening, and `graph::bfs_distance`'s `checked_add`, whose comment says why saturating would be worse — and `cur + 1` was not. Reproduced with the CLI: a program that sets `c["k"]` to `i64::MAX` and calls `c.inc("k")` dies with `attempt to add with overflow` / `fatal runtime error: failed to initiate panic, aborting` — the non-unwinding panic §10.4 forbids, so the host is aborted with no crash debugger and no diagnostic; in release it answers `-9223372036854775808`. Reachable from ordinary source, because a `Counter`'s values are ordinary `Int`s a program stores with `c[k] = n`. **D12 is answered** (per-wrapper totality), so it must fault — and `IntOverflow` already exists and is exactly this condition, so **no `FaultKind` was minted and no ABI bump spent**. The row moves to `AllocatesAndFaults`, which is a truth about the wrapper rather than a change in behaviour: a method call's fault check in `build.rs` is unconditional today, so the fault is observed either way, and what the row governs is what F17's verifier (S9) will read — which is REP-45's subject. One gate and one companion: `counter_inc_at_the_int_ceiling_faults_rather_than_wrapping` asserts the *fault kind* and is the gate (before the fix the test binary aborts), and `counter_inc_below_the_ceiling_still_counts` **passes on `main`** — it is a mutation companion, in `grid_rotate_left_then_right_restores_the_contents`'s role, ruling out a "fix" that faults on every `inc` rather than failing without the right one. |
| REP-44 | P2 | S | **done** (ADR-083) | rt | end-to-end semantics hunt | **`out()` rendered a whole-numbered `Float` identically to an `Int`, so output was not round-trippable.** `out(1.0)` printed `1`, `out(1e10)` printed `10000000000`, `1.0.to_text()` was `1`, and a `Vec[Float]` of `[3.0, 5.0]` printed `[3, 5]` — the same characters a `Vec[Int]` prints. The cause was one comment being right about the wrong language: "Rust's default `{}` formatting renders finite values in the shortest round-trippable form" — it does, for **Rust**, where `1` re-reads as an `f64`. §4.12's own typing rule is that `42` is strictly an `Int` literal and that `Float` and `Int` never mix, so `1` is not a `Float` here, and §4.12's "shortest round-trippable form" had been implemented as another language's version of that sentence. **ADR-083 decides the rule**: the rendered form is the shortest text that reads back as *the same `Float`*, so a finite value whose digits carry no `.` and no exponent takes `.0`, and `inf`/`-inf`/`NaN` — which §4.12 names — do not. `to_text()` and `out()` now share one function rather than two `format!` calls that happened to agree. Not only printing: §16.3 makes the rendered form a **sort key** (`Map`/`Set`/`Counter` order by it, ADR-066 decision 4, and `for x in s` iterates that order), which is also why the ADR rejects "print the type alongside". `run_pass_float_methods` asserted `"9"` and **pinned the defect; it is rewritten, not deleted** (§8.2). The gate is `a_whole_numbered_float_renders_as_a_float`: the existing descriptor test could not catch this because its one value is `2.5`, which carries a `.` and passes under either rule. |
| REP-45 | P2 | S | **done** | stdlib | runtime sweep | **`VecPush` and the two `DequePush*` rows claimed they cannot fault, and all three raise `TypeMismatch`.** `Effect::Allocates` makes `RuntimeSymbol::faults()` answer `false`, and all three call `adopt_or_reject`, which ends in `set_fault(ctx, TYPE_MISMATCH)`. The identical wrapper `praxis_grid_set` makes the same call and *is* declared `Faults`, so the manifest was internally inconsistent about one operation. What it costs: on a rejection the element is silently dropped, the wrapper answers the Unit sentinel, and the fault is observed by the next unrelated `CheckFault` — reporting a type mismatch at the wrong source location — or by the host after `main` returns, after the program has printed an answer from a collection that quietly lost a value. **S18's catalog sweep does not cover this** (it relates `AbiRet::Gc`/`GcUnit` to a row's result type), so the invariant is extended in S18's shape rather than the rows hand-corrected: `a_wrapper_that_can_raise_a_fault_declares_that_it_faults` reads `abi.rs` through `include_str!`, walks each definition, computes the fixed point of "can reach `set_fault`, directly or through a helper defined here", and asserts every such wrapper's row says it faults. **One direction only, deliberately** — a row may declare a fault the scan cannot see (the arithmetic wrappers are macro-generated and have no textual definition), so the test is weaker than the truth and never stricter, and the direction it checks is the one that produces wrong answers. `entry_reports_capabilities` asserted `!vec_push().can_fault()` with the reason "praxis_vec_push is Allocates, not AllocatesAndFaults" — it restated the row instead of checking it, which is what a bug-pinning test looks like when the bug is in a table — and is **rewritten, not deleted** (§8.2).  **Tightened after review**: the fixed point matched `set_fault(` as a plain substring over each body's raw text, comments included, so a *comment* inside a wrapper naming the helper — the most natural thing to write while removing a fault path — classified that wrapper as faulting and failed the assertion for a wrapper that cannot fault. Braces in comments had the same fragility from the other end: an unbalanced `}` in prose ended a body early. Both are latent rather than observed — no `praxis_*` body names the helper today — but a sweep a comment can fool is a sweep that gets edited around rather than satisfied, which is the one thing this invariant exists to prevent. `code_only` now strips a `//` comment from every line before the body text *and* the brace count, tracking string and `char` literals so `"//"` and `'/'` — both of which occur in the file — are not mistaken for comments. It is not a Rust lexer and says so: raw-string hashes and block comments are not modelled, because neither appears in a function body here. Gate: `the_manifest_sweep_reads_code_and_not_comments`, over synthetic source carrying exactly the fooling shape, plus `code_only_keeps_a_slash_inside_a_literal` for the stripper's own contract. |
| REP-46 | P2 | M | **done, with its open half recorded** | stdlib + rt | design-doc sweep | **§4.12's `wrapping_add`, `saturating_add` and `checked_add` did not exist.** The section says "integer arithmetic is checked by default … explicit alternatives" and then writes three methods a program could not call: `praxis check` clean (REP-33's asymmetry), `praxis run` `Y110` for each. So §4.12 described checked arithmetic with **no way to opt out**, and a program that wanted wrapping had to be written some other way while the document said it should not have to be. **Landed as exactly the three the document names** — three catalog rows on an `Int` receiver, three wrappers, three manifest rows — with `checked_add` answering a real `Option[Int]`, which S18/ADR-076 is what made spellable; before it this row could only have been another "for now the result is V" comment of the kind RT-14 was. **The `_sub`/`_mul` siblings are deliberately absent, and that is the row's open half**: §4.12 names only the `_add` trio, so nine rows would be inventing surface rather than implementing it, and the family's shape — three or nine, and whether `wrapping_neg` joins it — is a language decision nobody has made. It is written where a reader looks for it: beside the catalog rows, in §4.12 itself, and here. A half-invented numeric tower is harder to remove later than an honest gap. The gate `the_overflow_alternatives_answer_where_a_checked_add_faults` asserts at `i64::MAX`, where the ordinary `+` faults — two small numbers would pass against `praxis_int_add` itself and prove only that a name resolves — and asserts no fault is pending, which is the whole point of the three. |
| REP-47 | P2 | S | **done** (`Y023`, ADR-084) | hir-mono | front-end hunt | **A backtick template in value position was silently typed `Text` and lowered as a Text literal containing its own braces.** ``let t = `n = {int}` `` passed `praxis check` and `out(t)` printed `n = {int}` — the capture emitted as characters rather than reported. `parse_atom` accepted `BacktickTemplate` beside `IntLit`/`FloatLit`/`TextLit`; inference typed it `Text` "for now (a fresh var would be sounder but `Text` matches the eventual type)"; lowering built a `Lit::Text` of the raw interior. §7 defines a template **only** as a parser expression and §7.1 enters that sublanguage at `read` or `parse(text, …)` and nowhere else — **REP-34's boundary from the other side**, where four design-doc fences writing a labelled argument bare were the doc's mistake and not the grammar's. The difference is that here the language did not refuse, it *reinterpreted*, which is the worse failure because it produces an answer. **ADR-084 decides `Y023`, reported from inference** so `praxis check` sees it (REP-12's asymmetry; REP-26's `N008` is the precedent). Removing the token from `parse_atom` was the other candidate and gives up both the message and the recovery — `P001: expected an expression` at a backtick cannot say "you left out the word that gives this something to read from", and an unconsumed token cascades. The type is now a **fresh variable**, because the old comment had it backwards: `Text` is *plausible*, which is what let `out(t)` type-check and the program run. The gate `a_parser_template_in_value_position_is_reported` asserts the code, that ``read `n = {int}` `` is still clean — the sublanguage is entered by the word, so no template's meaning changed — and that the program produces **exactly one** error, the half that fails for a fresh variable with no report or a parser refusal with a cascade. `Y022` is deliberately left free; ADR-051 amended. |
| REP-48 | P2 | S | **done** (test hygiene) | rt | flake, this session | **`a_structural_key_hashes_by_contents_so_mutating_it_moves_its_bucket` was flaky**, and a repair effort whose whole discipline is "`just ci` is green at every commit" cannot carry a red that appears once a week and tells nobody why. It inserted one `DynamicKey` into a std `HashSet`, mutated the underlying `Vec` in place and asserted `!set.contains(&wrapped)` — but `wrapped` is the *same* `GcRef`, so `DynamicKey`'s equality is trivially true and the assertion rested entirely on the mutated key's new hash not probing the stored entry's slot. A one-element hashbrown table is a single 16-byte control group, so a new hash whose top seven bits match the stored tag lands on that slot, equality says yes, and `contains` answers `true`. **Measured on this tree: 163 false positives in 20000 iterations, 0.8%.** The rule it pins is real — D4/Y014 exists *because* a structural hash moves when its contents move, and `a_mutable_collection_is_not_a_key` is the compile-time half — so the fix is to the **observation**: the key is hashed with the same `RandomState` a `HashMap` builds its hasher from, before and after the push, and the two must differ. A second key with the same contents but a different identity must hash *equal*, which is the "by contents" half of the sentence and was not asserted at all. The `HashSet` stays only for the last line — the entry is still in the table, filed under a hash it no longer has, which is the shape of the corruption. |
| REP-50 | P2 | S | **done** | mir | reviewer, REP-36 … REP-49 review | **The `-0.0` float literal evaluated to `+0.0`.** `out(-0.0)` printed `0.0` and `1.0 / (-0.0)` answered `inf`, while the *computed* `-1.0 * 0.0` printed `-0.0` and divided to `-inf` — so the formatter was right and the evaluator was not, and the text a `Float` rendered to did not read back as that `Float`, which is the one rule **ADR-083** (REP-44's own ADR) exists to state. Not a rendering question at all: **ADR-083** had already decided that a Float's rendered text must read back as *that* Float, and `-0.0` and `0.0` are different Floats — so the language had taken a position on signed zero and the evaluator was losing a value it admits. (**An earlier telling of this row cited ADR-045 and had it backwards.** ADR-045 decides the opposite: its `compare` treats the two zeros as **equal**, and `f64::total_cmp` was rejected *for splitting them*. A `Map` given both keys holds one entry — measured. Rendering distinguishes them; ordering does not, and §4.12 now says so.) The cause is one line of MIR: `UnaryOp::Neg` on a `Float` lowered to `0.0 - x`, and `0.0 - 0.0` is `+0.0` under round-to-nearest. **Pre-existing** — `main` answers the same way, so REP-44 did not introduce it; REP-44 is what made it *visible*, because before ADR-083 both zeros printed `0`. **Landed as an instruction rather than a corrected constant**, the shape REP-37 set: `Inst::FloatNeg` is IEEE-754 `negate` and lowers to Cranelift's `fneg`, so "a negation is a subtraction from zero" is no longer spellable — the alternative, subtracting from `-0.0` instead, is exact for every input but is the same trick with a better constant, and it depends on the rounding mode. An `Int` negation is still `0 - x`: there the checked subtraction, and its fault at `Int::MIN`, is the right answer. §4.12 was silent on signed zero and now says this. The gate is `run_pass_float_negative_zero` (CLI, end to end), whose observation is `1.0 / x` and **not** `x == 0.0` — IEEE-754 says `-0.0 == 0.0`, so a gate written with `==` passes before the fix. It covers the literal, the same negation through a binding (which a lexer-level constant fold would leave broken), the involution `-(-0.0)`, and two ordinary values. `a_rendered_float_reads_back_as_the_same_float` (`scalars.rs`) states ADR-083 as a `to_bits` round trip at the rendering layer; it is REP-44's gate rather than REP-50's — before the rendering fix a whole-numbered Float printed like an Int, so `-0.0` rendered as `-0` and its last assertion was red — and its comment says so. |
| REP-51 | P1 | S | **done** | fe-cli-dbg + rt | reviewer, REP-36 … REP-49 review | **`praxis run` read standard input to EOF before the program started, so a program with no `read` in it hung against an open pipe.** §7.10 is precise — "The first `read` lazily reads standard input once into an immutable GC-managed source buffer" — and the host read it up front instead, installing the buffer as `input_source` before calling the entry function. Measured: `fn main() { out(2 + 2) }` with stdin bound to a FIFO a background writer holds open never finishes. That is the shape a **terminal** has and the shape a CI harness that leaves the descriptor open has, so *every* `praxis run` of a `read`-free program from a terminal hung; `< /dev/null` is what hid it, and it is what `Command::output` binds stdin to, which is why a 1500-test suite never saw it. **Pre-existing** (`main` hangs identically), and `run.rs`'s own doc comment had described the fix as already done for years: "The input is read lazily — only if the program contains a `read` expression — but for M6 we always read it upfront." **Landed as the laziness the sentence claims**, at the place §7.10 names: the host installs an `InputReader` (`praxis-runtime/src/input.rs`) rather than a buffer, and `praxis_get_input` — which is the first thing a `read` lowers to — calls it the one time. `take_input_reader` **removes** the reader, so "once" is structural rather than a flag. The reader is infallible by design: what an unreadable stdin *means* is the host's question, and the CLI answers it with the same message and the same exit code (2) it always did. `--input FILE` stays eager — a regular file cannot block, and reporting an unreadable one before any output is worth more than the symmetry. Two consequences recorded honestly: `GetInput`'s manifest row moves from `Pure` to **`AllocatesAndFaults`** (it allocates the buffer, and `praxis_alloc_text` faults `InvalidText` on input that is not UTF-8, §4.3), and `lower_read` gained the `check_fault` that makes that fault land at the `read` rather than at the next unrelated check — REP-52's shape, closed at this one site. **No ABI bump**: no field, no signature and no `FaultKind` was added. The gate is `a_program_that_never_reads_does_not_wait_for_standard_input`, which binds stdin to a pipe the test keeps open and **deadlines** rather than waiting, so a regression fails instead of wedging the suite. `a_program_that_reads_still_waits_for_its_input` is its mutation companion — same open pipe, a program that does `read`, and the child must still be running — because the cheapest way to stop a hang is to stop reading at all, and that would pass the gate. `a_second_read_sees_the_same_buffer` pins §7.10's "later `read` expressions reuse it". |
| REP-52 | P2 | M | **registered, not fixed** (belongs with F17/MIR-10, S9) | mir | reviewer, REP-36 … REP-49 review | **A fused pipeline's `collect` pushes with no fault check, so a `TypeMismatch` inside a pipeline is observed at the wrong place.** `crates/praxis-mir/src/build.rs`, `Sink::Collect` (the `emit_sink` arm that pushes the item) emits `Inst::Call { callee: CallTarget::Runtime(RuntimeSymbol::VecPush) }` and **no** `b.check_fault()` after it — unlike the `VecGet` two dozen lines above it in the same loop, which does check. `praxis_vec_push` returns the Unit sentinel with `pending_fault` set, the fused loop keeps running, and the fault surfaces at the next unrelated `CheckFault`: a type mismatch reported at the wrong source location, after the program has computed and possibly printed an answer from a collection that quietly lost a value — RT-10's shape, one layer up. **Reachable** when a collect target has already adopted an element type and a stage produces a different one. REP-45 corrected `VecPush`'s manifest row to `AllocatesAndFaults`, and **nothing in the tree reads that column here**: MIR consults `sym.faults()` only for the free-function helper lowerings (`build.rs` ~1166 and ~1188), and the method-call path emits an unconditional check anyway — which is REP-53. So the corrected row has no consumer yet, and this is one of the sites that will need it. The structural half belongs with **F17/MIR-10's verifier rule** "a faulting instruction is followed by a `CheckFault`" (S9), which is exactly what the corrected column exists to feed; adding one `check_fault` here without that rule fixes one site and leaves the class. Not fixed in this block: the block is runtime/stdlib defects, and this is the verifier's. |
...
| REP-56 | **P0** (filed P1) | M | **not done; every unsafe read it reached is bounded** (`044d0a4`, and the family finished since) | hir-mono | S20 repair (`afc6f3f`) | **A `choice` payload record's fields cannot be read, `praxis check` does not say so, and in a release build the program answered with an out-of-bounds heap read.** A `choice` case whose template has named captures produces an anonymous record; binding it and reading a field off it does not infer. `fn main() -> Int {\n  let ms = read scan(choice(M: `mul({a:int},{b:int})`))\n  var t = 0\n  for m in ms { t = t + match m { M(p) => p.a } }\n  t\n}` over `"mul(3,4)"` was **`error[Y112]: no field 'a' on this type`** as filed — and `praxis check` on the same file exits **0**, so this is REP-12's shape as well (the field is resolved at lowering, which `check` does not reach). `jit.rs` documents the gap at the test named "the anon-record-as-payload field access inference gap" and works around it with scalar payloads. **Re-severitied P1 to P0 by the final audit, and the earlier row's reasoning is what makes the case.** That row said "severity stays P1 because P0 in this register has meant a *silent* wrong answer", on the strength of a **debug** measurement: `rc=134`, `assertion left == right failed: int_payload reads eight bytes; Unit is 0 wide`, `internal error: a panic escaped the runtime wrapper praxis_int_load` — REP-37's width guard and ADR-080's `catch_unwind` doing their jobs, the read caught rather than performed. **In release the guard was not there.** It was a `debug_assert`, which is compiled out, and what was left was `unsafe { *r.payload::<i64>() }` against a descriptor **zero bytes wide**. Measured on a release `praxis-cli`: `check` `rc=0`, and three consecutive runs printed three different pointer-shaped numbers with `rc=0` and no diagnostic where the answer is `3`. So it *is* a silent wrong answer, from a program that passes `check`, and it is an 8-byte out-of-bounds heap read besides — P0 by this register's own definition, and memory-unsafe on top of it. **Every measurement in this repair round was taken in debug, and `just ci` builds only debug**, which is why twenty-one stages of a green suite could not see it. **What landed here, and what did not.** The *read* is bounded: `int_payload`'s width check is an ordinary branch in every profile (`044d0a4`), so the release build takes ADR-080's defined panic path instead of reading past the object. **Bounding the read was chosen over refusing the program at analysis** because the unsound read is a property of `int_payload` — reachable by any route that puts a narrow payload in an arithmetic wrapper — and a diagnostic for this one program would have left the read unbounded for every other. **The row itself is still open**: the payload record's *type* is what it is about, and a program that passes `check` still dies (a defined abort with a message now, rather than a random number). The remainder owed is the anonymous record's type reaching the field read, and a `check`-time report for it — REP-12's asymmetry, at this site. **Why S20 did not fix it:** it is an inference/lowering defect in the payload's *type*, not in the parser interpreter, and the parser produces the right record — S20's own IPR-14 pressure test reads its values through single-anonymous-capture templates for exactly this reason. It makes §7.5's most natural `scan(choice(template))` spelling unusable, which is the `m9_noisy_scan` shape. **The bound was half-landed and a reviewer caught it.** `044d0a4` made `int_payload`'s check an ordinary branch and stopped there, leaving its three siblings in the same block — `float_payload` (8 bytes), `praxis_char_load` (4) and `praxis_bool_load` — reading unchecked in release, which is the same defect in three more places. `praxis_bool_load` was worse than unbounded: it materialized a Rust `bool` from `*r.payload::<bool>()`, and a `bool` whose byte is not 0 or 1 is an *invalid value*, so that was undefined behaviour independently of the read's bounds. **All four now go through the `read_scalar(r, Payload<T>)` reader S23 already built for REP-37**, which proves descriptor *identity* rather than merely width, so a caller cannot pick a width at all. Fixing them one at a time is what let three of four survive the first repair, and the new gate is written accordingly: `every_scalar_payload_read_goes_through_the_bounded_reader` scans the crate's non-test, non-comment code for any direct scalar payload read, so it fails for a *fifth* site nobody has written yet. Verified red by reverting `float_payload` alone. Measured in a **release** build, 50 runs of the row's own repro: before, three different nine-digit numbers with `rc=0`; after, one identical defined refusal naming `Int` against `Unit` and `rc=134`. **Still open**: the payload record's type does not reach the field read, so `praxis check` still exits 0 on a program that then aborts. A defined abort is bounded, not good. |
...
| REP-60 | P2 | S | **`--input` half done; stdin half registered** | fe-cli-dbg | final audit | **A zero-byte input file makes any `read` fault with a diagnostic that has no offset, no `expected` and no `actual`.** `printf '' > e.in`, `let r = read lines(int)` / `out(r.len())`, `praxis run --debug never --input e.in e.px` is `error: program faulted: input parse mismatch` plus a backtrace and **nothing else** — no position, no expectation, no mention that the file is empty. A single newline is fine (`0`). The cause is not the parser: `crates/praxis-cli/src/run.rs` installed the buffer only `if !t.is_empty()`, so `ctx.input_source` stayed the immortal `Unit`; `Input::new` (`crates/praxis-runtime/src/parser.rs`) answers `None` for a non-Text source and takes the `clear_parse_detail` path, documented as the "no detail was recorded" case. So the one shape a reader could least guess at was the one shape that carried nothing. **Fixed for `--input`**: the `Text` is allocated unconditionally, so `read` runs against a zero-length buffer and the constructors answer from their own rules — `lines(int)` over it is `[]` under `split_lines`'s rule, and one that requires content faults at `0..0` naming what it expected. A zero-byte file is *input*, not the absence of input. Gate: `a_zero_byte_input_file_is_empty_input_and_not_a_contentless_fault` (`crates/praxis-cli/tests/run.rs`), which exits 1 before the fix. **The stdin twin is registered, not fixed**: `praxis_get_input` carries the same `if !bytes.is_empty()` guard, so `praxis run e.px < /dev/null` still gives the contentless message, and that wrapper's doc comment states the behaviour as the design ("Empty input leaves `input_source` at the immortal Unit, which is the state 'no input' has always had"). That is a runtime contract with an ABI-boundary guard behind it rather than a CLI line, and changing it is the pass that decides whether "no input at all" and "empty input" are one state or two — which §7.10 does not say. Take both halves together and delete this row. |
| REP-61 | P2 | S | **done** | hir-mono | user report | **A type mismatch named the operand as the requirement and a type nobody wrote as the mistake.** `let a = "asd"` / `let b = "qwe"` / `let c = a + b` reported `error[Y001]: expected Text, found Int` **twice** — pointing at two `Text` bindings and naming `Int`, which appears nowhere in the program. The halves were swapped. `TypeDb::unify` builds `Mismatch { expected, found }` in argument order and `type_mismatch` renders it as written, so the orientation is the *caller's*, and five call sites passed the operand first: binary arithmetic (both operands), the range bounds, the unary operators, and `parse(text, …)`'s text argument. Measured on all five — `!1` said `expected Int, found Bool`, `parse(1, int)` said `expected Int, found Text`. **Fixed at the five call sites, not inside `unify`**: `var x = 0; x = "hi"` was already correct (`unify(existing, rhs)`), and so were the annotation, argument, iterator-item and catalog-bound sites, so flipping the primitive would have broken more than it fixed. `unify`'s parameters are **named** `expected`/`found` now rather than `a`/`b`, which is what makes the orientation visible at a call site — the reason five of them could drift in the first place. Gate: `a_mismatch_names_the_requirement_as_expected_and_the_program_as_found` (`infer_tests.rs`), one row per site plus the already-correct `var` case as the control, so a "fix" applied inside `unify` turns it red. |
| REP-62 | P3 | S | **done** | fe-cli-dbg | user report, REP-61 | **The `path:line:col` header printed a 0-based column beside a 1-based line.** An error on a file's very first character read `f.px:1:0`, and every header disagreed by one with the caret drawn under it — and with `rustc`, `gcc` and every other compiler's `line:col`. `LineCol::col` counts from zero *deliberately* (§`line_map`: "the first byte of a line is column 0"), so `linecol_to_offset` can invert it; the defect was `snippet.rs` printing that number raw. **Fixed at the one place a column is printed**, with `LineMap` untouched — the caret padding still uses the 0-based number, because it is a count of characters to skip rather than a position. Gate: `a_header_column_is_one_based_and_lands_on_the_caret`, written as a *relation* — it parses the column back out of the header, indexes the source line with it, and checks the caret's own offset against it — so it survives a reworded header, where the eight literal snapshots it sits beside would not. |
| REP-63 | P2 | S | **done** | fe-cli-dbg | user report, REP-61 | **A node's `text_range` included the whitespace in front of it, so every caret under an expression started a column early and ran a column wide.** In `let c = a + b` the `PATH_EXPR` for `a` spanned `" a"`, which renders as a two-character underline beginning under the `=` — the diagnostic looked like it was pointing at the operator. `start_node` opened the node and then let `bump`'s own trivia sweep pull the whitespace *inside* it. **The one arm that worked around it is why this was invisible**: `parse_atom`'s `IntLit`/`FloatLit`/`TextLit`/`BacktickTemplate` case ate trivia first, with a comment saying why — so `1 + "y"` underlined `"y"` exactly while `a + b` underlined `" a"`, and a test written with literals could not see it. `true`/`false` were a *separate arm* precisely because they were on the wrong side of it. **Fixed in `start_node` and `checkpoint_lhs`** rather than at 54 call sites; the root is the single exception (`start_root_node`), since a file's leading trivia has nowhere else to go. The literal arms collapse into one. Gates: `a_node_never_begins_with_trivia` asserts the invariant over whole trees for twelve programs including two recovery paths — a snapshot pins only the shapes a fixture reaches — and `an_operands_range_is_the_operand_and_not_the_space_before_it` states the consequence directly. |
| REP-64 | P1 | S | **done** | mir | ADR-085 work | **A compound assignment on a `Float` adds two bit patterns as integers.** `var f = 1.0` / `f += 2.0` / `out(f)` prints **`9218868437227405312`**. Both compound-assignment sites in `crates/praxis-mir/src/build.rs` (the local at ~727 and the indexed store at ~787) call `lower_int_binop(b, op_to_int_binop(*op), …)` unconditionally: there is no `op_to_float_binop` in the tree at all, and floats ride the `i64` channel as their bit pattern (ADR-037), so the addition is on the pattern. **A silent wrong answer** — no fault, no diagnostic — which is P0's shape in this register; it is P1 only because `f = f + 2.0` is correct, so the wrong answer needs the compound spelling. **Pre-existing**: `git show HEAD:…/build.rs` has the same two unconditional calls, and TY-26/REP-50 both touched `Float` without reaching this path. Found while adding ADR-085's `Text` branch to the same two lines. **Not fixed here**: the answer is a `Float` branch in both sites mirroring `lower_float_binop`/`lower_materialize_float` (which already exist, for `a + b`), plus a decision about `%` — inference rejects `%` on `Float` (TY-27) and `op_to_int_binop` still maps `RemAssign`, so `f %= 2.0` needs the `Y016` the binary form gets. That is a `Float` correctness row and the `Text` branch that found it deliberately did not land it. **Fixed since**: an `ArithVia` discriminant chosen from the target's static type threads through both compound-assignment sites, with `lower_extract_float`/`lower_materialize_float` as the bit-cast pair — ADR-037's rule applied at the two sites that forgot it. `%=` on a `Float` is now the `Y016` the binary form gives. Measured after: `+=`/`-=`/`*=`/`/=` answer 3.0/7.5/6.0/4.5, a subscript target (`m["a"] += 2.0` on a `Map[Text, Float]`) answers 3.0, and `Int` compound assignment is unchanged. A **field** target is still `Y021` — a field is not an assignable place in this language, which is pre-existing and not this row's. **Closed since (ADR-124):** a field *is* a place, `p.x += 1` goes through the same `lower_compound_arith` this row installed, and the float shape test carries a field target beside the binding, closure and subscript ones. Related: `Int.to_text()` does not exist (only `Float` has one), so no number can be turned into a `Text` in any spelling — ADR-085 records that as its own gap. |
...
None. This stage is empty: every verified finding is CONFIRMED or PARTIAL, and
**§1's table is where the breakdown lives** — it said `136/3` here and `135/4`
there, and the fourth `PARTIAL` (FE-08) was missing from this sentence's list
from the plan's first commit. No auditor returned REFUTED, ALREADY-FIXED or
UNCERTAIN. The `PARTIAL` findings are assigned to real stages (TY-24 to
Stage 13, MIR-10 to Stage 9, IP-11 to Stage 19, FE-08 to Stage 2) because in
each case the verification confirmed a genuine underlying defect while narrowing
the audit's overstated claim: TY-24's silent overwrite is real but
ScopeTree::bind's doc/signature mismatch is the actual defect; MIR-10's verifier
is absent but the scalar-across-safepoint rule would initially fail on the eager
lower_seq_* lowerers; IP-11's missing atomics are real but `uint` has no runtime
descriptor and should not ship; FE-08's fuzz generators are indeed UTF-8-only,
but that is test coverage rather than a product defect.
