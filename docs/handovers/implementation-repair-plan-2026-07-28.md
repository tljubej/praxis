...
**One row is `unscheduled`: REP-15**, a P0 that arrived after every stage that
could own it was written. It needs a design decision and a vertical slice of its
own — a new stage, on the model of D6/S17's `Range` work — so scheduling it is a
session's first decision, not a line in this table.
