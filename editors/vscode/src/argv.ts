...
//
// **This file is everything about the command line that can be tested without a
...
/** The `praxis` subcommands the extension's file commands invoke. */
export type PraxisSubcommand = "run" | "check";

/** The `--input FILE` option `praxis run` takes: read the process input from a
...
export function fileCommandArgv(
  ...
  // `--input` is `run`'s alone. `check` never reads input, so appending it
  // there would be an argument the CLI rejects rather than one it ignores.
 ...
 *
 * A command runs in an integrated terminal rather than an output channel
 * because the crash debugger is **interactive** — an output channel is
 * write-only, so a program that faults would print its prompt into a pane that
 * cannot answer it.
 *
